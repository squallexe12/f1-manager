...
**Last updated:** 2026-05-06 (IP-09a Drivers redesign — schema v13, adds `driver.careerWins`, `driver.careerPodiums`, `driver.careerStarts`, `driver.worldTitles`, `driver.pulse`, `driver.portraitUrl`, `driver.scoutSignal`, `driver.scoutingReports`)
**Status:** Active
...
### Current state
- `SCHEMA_VERSION = 13`
- `MIGRATIONS = { 1: v1→v2, 2: v2→v3, 3: v3→v4, 4: v4→v5, 5: v5→v6, 6: v6→v7, 7: v7→v8, 8: v8→v9, 9: v9→v10, 10: v10→v11, 11: v11→v12, 12: v12→v13 }`
  ...
  - v12→v13 adds eight persisted driver fields for the IP-09 Drivers Page redesign. On every `driver`: `careerWins` ← `0`, `careerPodiums` ← `0`, `careerStarts` ← `0`, `worldTitles` ← `0` (running totals across seasons; `applyRaceCareerDeltas()` and `applySeasonEndCareerDeltas()` in `src/engine/drivers/career-stats.ts` accumulate them inside the existing `lastProcessedRound` guard); `pulse` ← `{ headline: '', detail: '' }` (per-driver narrative status, regenerated each round and at game init by `derivePulse()` in `src/engine/drivers/pulse.ts`; 13-branch deterministic template); `portraitUrl` ← `null` (optional driver portrait URL; null = render stripe SVG placeholder); `scoutSignal` ← `'available'` (`'hot' | 'tracking' | 'available'` derived by `computeScoutSignal()` in `src/engine/drivers/scout-signal.ts`; meaningful only for free agents/F2 but computed for every driver for type safety); `scoutingReports` ← `0` (count of player-filed scouting reports, incremented via `gameStore.fileScoutingReport()` thin-dispatch action which calls `applyScoutingReport()` in `src/engine/drivers/apply-scouting-report.ts`). Existing saves do not retroactively get real-world numbers — only fresh games seeded via `src/data/drivers.ts` carry curated EOS-2025 values (Verstappen 4×/64w/116p/218s, Hamilton 7×/105w/202p/356s, Alonso 2×/32w/106p/406s, etc.; rookies and NPCs at 0/0/0/0). After defaulting, the migration runs `derivePulse()` and `computeScoutSignal()` over every driver so loaded saves render correctly without waiting for the next post-race tick. **Branch 11 ("Locked in") threshold drift:** spec §3.2 documents `mood.confidence >= 80 && mood.motivation >= 80` but the shipped implementation uses `>= 85` to resolve a test-fixture conflict where the default base driver (mood 80/80) was reaching branch 11 ahead of the intended branch-13 fallback. Drivers with confidence/motivation in the 80–84 range therefore do not display "Locked in" and instead show "Midfield grind" (P11+) or "Chasing form" (P10 and better). Tracked here as a known intentional drift; revisit if the spec is amended.
