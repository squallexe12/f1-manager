...
`pulse`, `scoutSignal`, `scoutingReports` are **not** seeded in `data/drivers.ts`; they are computed at game init by the orchestrator/state-manager.
...
All composition helpers (`composeRoster`, `computePeerAttributes`, `computeChampionshipSummary`, `buildRivalryIndex`, `scoutScore`) live in `src/lib/utils/drivers-page.ts` and are unit-tested independently. They contain **no game logic** — they are pure presentation derivations.

### 4.6 Store actions

- `fileScoutingReport(driverId)` — added in IP-09a (§3.5).
- `approachDriver(driverId)` — if a matching action already exists, reuse it; otherwise add a thin stub action that emits a toast `"Approach tabled · negotiation flow coming soon"` and returns. The stub does not modify `world` so it does not trigger autosave.
- `openContractNegotiation(driverId)` — same treatment as `approachDriver`: reuse if present, stub otherwise.
...
1. **Real-world seed values for non-headline drivers** — values for drivers like Stroll, Tsunoda, Magnussen are estimates from public records. Pre-implementation, we'll cross-check against statsf1.com/wikipedia and revise. If a value is uncertain to within ±5 we round to the nearest 5; the spec accepts this fuzz.
