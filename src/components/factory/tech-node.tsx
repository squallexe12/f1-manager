...
export function TechNode({ upgrade, onStart, onPause, className = '' }: TechNodeProps) {
  ...
  return (
    <div
      className={`
        ...
        ${isLocked ? 'border-[var(--border-default)] opacity-40' : 'border-[var(--border-default)]'}
        ${!isLocked && !isComplete && !isInProgress ? 'bg-[var(--bg-surface)]' : ''}
        ${className}
      `}
    >
